...
var postRunDone=false;var theDomLoaded=false;var gameReadyToStart=false;var requestedCanvas=false;var gameStarted=false;var updatedScreenSize=false;var loadProgressFrac=0;var fakeProgressPercentStart=80+Math.random()*15;window.addEventListener('DOMContentLoaded',domContentLoaded);window.addEventListener('load',function(){console.log("Load event received");if(inIframe()){document.addEventListener('click',ev=>{let canvas=document.getElementById('canvas');if(canvas){canvas.focus();}});window.addEventListener("keydown",function(e){if(["Space","ArrowUp","ArrowDown","ArrowLeft","ArrowRight"].indexOf(e.code)>-1){e.preventDefault();}},false);}});function touchStart(ev){ev.preventDefault();for(let i=0;i<ev.changedTouches.length;i++){let t=ev.changedTouches.item(i);_touch_start(t.identifier);_touch_move(t.identifier,t.clientX,t.clientY);}}
...
var startGameAttempts=0;var tryStartGameTimeout=null;function tryStartGame(){console.log("tryStartGame()",gameReadyToStart,theDomLoaded,postRunDone);if(gameReadyToStart){return;}
if(!postRunDone||!theDomLoaded){startGameAttempts++;if(startGameAttempts==20){return;}
